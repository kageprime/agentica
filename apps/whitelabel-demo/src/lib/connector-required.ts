...
/**
 * @param options.wrapperMode Whether this app is talking to the platform under ONE
  ...
  if (remedy === 'own_account') {
    if (options.wrapperMode === true) {
      return {
        ...
        explanation: `This session needs ${name} and cannot start without it. ${name} authorizes one person at a time, and sessions here run under this app's own credential rather than yours — so there is nothing it can connect on your behalf, and a link would only ever reconnect that same account.`,
        unblockedBy: [
    ...
    if (options.wrapperMode === false) {
      return {
        headline: `Connect ${name} in your account`,
        explanation: `This session needs ${name} and cannot start without it. ${name} authorizes one person at a time, so it has to be connected by the account this app is signed in with — a connection that lives in the platform, and that this app has no screen for making.`,
        unblockedBy: [`Connect ${alias} from your dashboard, then retry.`],
        canMintConnectLink: false,
      };
    }
    return {
