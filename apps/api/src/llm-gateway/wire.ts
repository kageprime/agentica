...
const AUTH_DESCRIPTION =
  ...
  'POST /v1/projects/{projectId}/gateway/keys) or an account token ' +
...
function chatCompletionsRoute(path: string) {
  ...
  return createRoute({
    ...
    description: `OpenAI-compatible chat completions, proxied through the LLM gateway (model routing/failover, budgets, usage billing, and request tracing all apply). The body is forwarded close to verbatim to the resolved upstream provider.\n\nAuth: ${AUTH_DESCRIPTION}\n\n\`\`\`\ncurl -sS $KORTIX_API_URL${fullPath} \\\n  -H "Authorization: Bearer $KORTIX_GATEWAY_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d \'{"model":"claude-sonnet-4-5","messages":[{"role":"user","content":"Say hello in one word."}]}\'\n\`\`\``,
    ...auth,
    request: {
      body: {
        required: true,
