...
const CAPABILITY_HINTS: Record<string, Partial<ItemCapabilities>> = {
  elevenlabs: { secrets: ['ELEVENLABS_API_KEY'], network: ['api.elevenlabs.io'] },
  replicate: { secrets: ['REPLICATE_API_TOKEN'], network: ['api.replicate.com'] },
...
export function capabilitiesOf(item: RegistryItem): ItemCapabilities {
  ...
  const merge = (...lists: Array<string[] | undefined>) => [...new Set(lists.flatMap((l) => l ?? []))];
  return {
...
const CURATED_BUNDLES: RegistryItem[] = [
  {
    ...
    title: 'Research Pack',
    description: 'Everything for evidence-based research: deep research, cited reports, and academic paper search.',
  ...
  },
  {
    ...
    type: 'registry:bundle',
    title: 'Sales Pack',
    description: 'Outbound-ready: account research, outreach drafting, call prep, and competitive analysis.',
    categories: ['bundle', 'sales'],
    registryDependencies: ['account-research', 'draft-outreach', 'call-prep', 'competitive-analysis'],
    meta: { source: 'kortix' },
  },
  {
    name: 'documents-pack',
...
const CACHE: MarketplaceCache = ((globalThis as unknown as {
...
function refsFromSource(src: MarketplaceSource): RegistryRef[] {
  ...
  if (base.kind === 'github') {
    return paths.map((p) => ({ ...base, ref: src.gitRef || base.ref, subdir: p ?? base.subdir }));
  }
  if (base.kind === 'local') {
    return paths.map((p) => ({ ...base, path: p ? `${base.path.replace(/\/+$/, '')}/${p}` : base.path }));
  }
...
function startExternalBuild(): Promise<Catalog> {
  if (CACHE.external && Date.now() - CACHE.externalAt < EXTERNAL_TTL_MS) return Promise.resolve(CACHE.external);
...
async function buildExternalCatalog(): Promise<Catalog> {
  ...
  CACHE.gen++;
  const acc: Catalog = CACHE.external ? { items: [], byId: new Map() } : (CACHE.partial ??= { items: [], byId: new Map() });
...
function addRegistryToCatalog(acc: Catalog, registry: LoadedRegistry, ref: RegistryRef, sourceId?: string): void {
  const registryName = registry.registry.name || describeRegistry(ref);
  const sourceUrl = ref.kind === 'github' ? `https://github.com/${ref.owner}/${ref.repo}` : undefined;
...
export async function listMarketplaces(): Promise<MarketplaceFacet[]> {
  ...
  return [...by.values()].sort((a, b) => (a.id === 'kortix' ? -1 : b.id === 'kortix' ? 1 : a.label.localeCompare(b.label)));
}
...
export const FEATURED_MARKETPLACES: FeaturedMarketplace[] = [
  ...
  { address: 'anthropics/knowledge-work-plugins', label: 'Anthropic Knowledge Work', owner: 'anthropics', description: 'Official Anthropic — ~212 knowledge-work skills', license: 'Apache-2.0' },
  { address: 'openai/role-specific-plugins', label: 'OpenAI Role Plugins', owner: 'openai', description: 'Official Codex role-specific plugins', license: 'MIT' },
  { address: 'wshobson/agents', label: 'wshobson Agents', owner: 'wshobson', description: '156 skills · 192 agents · 102 commands', license: 'MIT' },
  // Big, permissive, reputable (verified counts, 2026-06-16)
  { address: 'davila7/claude-code-templates', label: 'aitmpl Templates', owner: 'davila7', description: '871 skills · 449 agents · 388 commands', license: 'MIT' },
  { address: 'alirezarezvani/claude-skills', label: 'Claude Skills', owner: 'alirezarezvani', description: '767 skills + agents + commands', license: 'MIT' },
  { address: 'github/awesome-copilot', label: 'Awesome Copilot', owner: 'github', description: 'GitHub/Microsoft official — 526 skills, 115 agents', license: 'MIT' },
  { address: 'microsoft/azure-skills', label: 'Azure Skills', owner: 'microsoft', description: 'Microsoft official Azure skills (68)', license: 'MIT' },
  { address: 'mattpocock/skills', label: 'Matt Pocock Skills', owner: 'mattpocock', description: 'TypeScript & web skills (skills.sh top author)', license: 'MIT' },
  { address: 'davepoon/buildwithclaude', label: 'Build with Claude', owner: 'davepoon', description: '334 skills · 363 agents · 414 commands', license: 'MIT' },
  { address: 'secondsky/claude-skills', label: 'Claude Skills (secondsky)', owner: 'secondsky', description: '212 skills · 53 agents · 65 commands', license: 'MIT' },
  { address: 'K-Dense-AI/scientific-agent-skills', label: 'Scientific Agent Skills', owner: 'K-Dense-AI', description: '147 science skills', license: 'MIT' },
  { address: 'anthropics/claude-plugins-official', label: 'Anthropic Official Plugins', owner: 'anthropics', description: '28 curated first-party plugins', license: 'Apache-2.0' },
  { address: 'obra/superpowers', label: 'Superpowers', owner: 'obra', description: 'Brainstorming, debugging, TDD (Jesse Vincent)', license: 'MIT' },
  { address: 'garrytan/gstack', label: 'gstack', owner: 'garrytan', description: "Garry Tan's Claude Code setup — 62 skills", license: 'MIT' },
  { address: 'SuperClaude-Org/SuperClaude_Framework', label: 'SuperClaude Framework', owner: 'SuperClaude-Org', description: '35 personas, commands & skills + MCP', license: 'MIT' },
  // Verticals
  { address: 'kostja94/marketing-skills', label: 'Marketing Skills', owner: 'kostja94', description: '172 marketing skills — SEO, social, ads', license: 'MIT' },
  { address: 'AgricIDaniel/claude-seo', label: 'Claude SEO', owner: 'AgricIDaniel', description: 'Technical SEO, schema, GEO/AEO', license: 'MIT' },
  { address: 'transilienceai/communitytools', label: 'Security Community Tools', owner: 'transilienceai', description: 'Security, pentest & bug-bounty skills', license: 'MIT' },
  { address: 'cloudflare/skills', label: 'Cloudflare Skills', owner: 'cloudflare', description: 'Workers & Agents SDK (official)', license: 'Apache-2.0' },
  // Large / vendor — license caveats (all-rights-reserved; we fetch at install, never re-host)
  { address: 'ComposioHQ/awesome-claude-skills', label: 'Composio Awesome Skills', owner: 'ComposioHQ', description: '864 community skills (Composio)', license: '' },
  { address: 'openai/plugins', label: 'OpenAI Codex Plugins', owner: 'openai', description: '551 official Codex skills — per-plugin terms', license: '' },
  { address: 'vercel-labs/agent-skills', label: 'Vercel Agent Skills', owner: 'vercel-labs', description: 'React, web design & more', license: '' },
  // Official vendor sources (indexed by skills.sh — recognizable brands)
  { address: 'google/skills', label: 'Google Skills', owner: 'google', description: 'Agent skills for Google products', license: 'Apache-2.0' },
  { address: 'huggingface/skills', label: 'Hugging Face Skills', owner: 'huggingface', description: 'The Hugging Face ecosystem for agents', license: 'Apache-2.0' },
  { address: 'stripe/ai', label: 'Stripe AI', owner: 'stripe', description: 'Build AI products with Stripe', license: 'MIT' },
  { address: 'supabase/agent-skills', label: 'Supabase Agent Skills', owner: 'supabase', description: 'Agent skills for Supabase', license: 'MIT' },
  { address: 'firebase/agent-skills', label: 'Firebase Agent Skills', owner: 'firebase', description: 'Agent skills for Firebase', license: 'Apache-2.0' },
  { address: 'dotnet/skills', label: '.NET Skills', owner: 'dotnet', description: 'Skills for .NET & C# coding agents (105)', license: 'MIT' },
  { address: 'flutter/skills', label: 'Flutter Skills', owner: 'flutter', description: 'Agent skills for Flutter', license: 'BSD-3-Clause' },
  { address: 'googleworkspace/cli', label: 'Google Workspace CLI', owner: 'googleworkspace', description: 'Workspace CLI + 95 agent skills', license: 'Apache-2.0' },
  { address: 'forcedotcom/sf-skills', label: 'Salesforce Skills', owner: 'forcedotcom', description: 'Agentforce / Salesforce skills (69)', license: '' },
  { address: 'oracle/skills', label: 'Oracle Skills', owner: 'oracle', description: 'Curated skills for Oracle tech', license: 'UPL-1.0' },
  { address: 'dbt-labs/dbt-agent-skills', label: 'dbt Agent Skills', owner: 'dbt-labs', description: 'Agent skills for dbt workflows', license: 'Apache-2.0' },
  { address: 'duckdb/duckdb-skills', label: 'DuckDB Skills', owner: 'duckdb', description: 'Agent skills for DuckDB', license: 'MIT' },
  { address: 'langchain-ai/langchain-skills', label: 'LangChain Skills', owner: 'langchain-ai', description: 'Agent skills for LangChain', license: '' },
  { address: 'trailofbits/skills', label: 'Trail of Bits Skills', owner: 'trailofbits', description: 'Security research & audit skills (74)', license: 'CC-BY-SA-4.0' },
  { address: 'tavily-ai/skills', label: 'Tavily Skills', owner: 'tavily-ai', description: 'Agent skills for Tavily search', license: 'MIT' },
  { address: 'brightdata/skills', label: 'Bright Data Skills', owner: 'brightdata', description: 'Web-data & scraping skills', license: 'MIT' },
  { address: 'posit-dev/skills', label: 'Posit Skills', owner: 'posit-dev', description: 'R & Python skills from Posit', license: 'MIT' },
  { address: 'callstackincubator/agent-skills', label: 'React Native Skills', owner: 'callstackincubator', description: 'Agent-optimized React Native skills', license: 'MIT' },
  { address: 'vuejs-ai/skills', label: 'Vue Skills', owner: 'vuejs-ai', description: 'Agent skills for Vue 3', license: 'MIT' },
  { address: 'analogjs/angular-skills', label: 'Angular Skills', owner: 'analogjs', description: 'Agent skills for Angular (AnalogJS)', license: 'MIT' },
  // Notable authors & big collections
  { address: 'antfu/skills', label: "Anthony Fu's Skills", owner: 'antfu', description: 'Curated agent skills (Anthony Fu)', license: 'MIT' },
  { address: 'mitsuhiko/agent-stuff', label: 'Agent Stuff', owner: 'mitsuhiko', description: "Armin Ronacher's commands & skills", license: 'Apache-2.0' },
  { address: 'addyosmani/web-quality-skills', label: 'Web Quality Skills', owner: 'addyosmani', description: 'Web performance & quality (Addy Osmani)', license: 'MIT' },
  { address: 'danielmiessler/Personal_AI_Infrastructure', label: 'Personal AI Infrastructure', owner: 'danielmiessler', description: '458 skills — agentic AI infra (PAI)', license: 'MIT' },
  { address: 'HKUDS/CLI-Anything', label: 'CLI-Anything', owner: 'HKUDS', description: 'Make any software agent-native (150)', license: 'Apache-2.0' },
  // More vendor/brand sources from skills.sh
  { address: 'google/agents-cli', label: 'Agents CLI', owner: 'google', description: 'CLI + skills for any coding agent (Google)', license: 'Apache-2.0' },
  { address: 'google-gemini/gemini-skills', label: 'Gemini Skills', owner: 'google-gemini', description: 'Skills for the Gemini API & models', license: 'Apache-2.0' },
  { address: 'google-labs-code/stitch-skills', label: 'Stitch Skills', owner: 'google-labs-code', description: 'Agent skills for Google Stitch', license: 'Apache-2.0' },
  { address: 'greensock/gsap-skills', label: 'GSAP Skills', owner: 'greensock', description: 'Official AI skills for GSAP', license: 'MIT' },
  { address: 'larksuite/cli', label: 'Lark / Feishu CLI', owner: 'larksuite', description: 'Official Lark/Feishu CLI + skills', license: 'MIT' },
  { address: 'heygen-com/hyperframes', label: 'HyperFrames', owner: 'heygen-com', description: 'Write HTML, render video (HeyGen)', license: 'Apache-2.0' },
  { address: 'vercel-labs/json-render', label: 'JSON Render', owner: 'vercel-labs', description: 'Generative UI framework (Vercel)', license: 'Apache-2.0' },
  { address: 'millionco/react-doctor', label: 'React Doctor', owner: 'millionco', description: "Catches your agent's bad React", license: 'MIT' },
  { address: 'firecrawl/cli', label: 'Firecrawl CLI', owner: 'firecrawl', description: 'CLI + skill for Firecrawl scraping', license: '' },
  { address: 'apify/agent-skills', label: 'Apify Agent Skills', owner: 'apify', description: 'Web scraping & automation (Apify)', license: '' },
  { address: 'better-auth/skills', label: 'Better Auth Skills', owner: 'better-auth', description: 'Agent skills for Better Auth', license: '' },
  // Famous authors
  { address: 'twostraws/SwiftUI-Agent-Skill', label: 'SwiftUI Skill (Paul Hudson)', owner: 'twostraws', description: 'SwiftUI agent skill — Hacking with Swift', license: 'MIT' },
  { address: 'AvdLee/SwiftUI-Agent-Skill', label: 'SwiftUI Best Practices', owner: 'AvdLee', description: 'Expert SwiftUI skill (Antoine van der Lee)', license: 'MIT' },
  { address: 'Dimillian/Skills', label: "Dimillian's Skills", owner: 'Dimillian', description: 'Codex skills (Thomas Ricouard)', license: 'MIT' },
  { address: 'Nutlope/hallmark', label: 'Hallmark', owner: 'Nutlope', description: 'Anti-AI-slop design skill (Hassan)', license: 'MIT' },
  { address: 'ibelick/ui-skills', label: 'UI Skills', owner: 'ibelick', description: 'Skills for design engineers (ibelick)', license: 'MIT' },
  { address: 'chrisbanes/skills', label: "Chris Banes' Skills", owner: 'chrisbanes', description: 'Kotlin, Compose & Android skills', license: 'Apache-2.0' },
  { address: 'hamelsmu/evals-skills', label: 'Evals Skills', owner: 'hamelsmu', description: 'Skills for AI evals (Hamel Husain)', license: 'MIT' },
  { address: 'mrgoonie/claudekit-skills', label: 'ClaudeKit Skills', owner: 'mrgoonie', description: 'All ClaudeKit.cc skills (45)', license: '' },
  { address: 'JimLiu/baoyu-skills', label: 'Baoyu Skills', owner: 'JimLiu', description: "Baoyu's agent skills collection", license: '' },
  { address: 'nuxt-content/docus', label: 'Docus', owner: 'nuxt-content', description: 'Docs skills with Nuxt Content', license: 'MIT' },
  // Big / hyped collections (high install counts on skills.sh)
  { address: 'nexu-io/open-design', label: 'Open Design', owner: 'nexu-io', description: '513 skills — local-first design system', license: 'Apache-2.0' },
  { address: 'ruvnet/ruflo', label: 'ruFlo', owner: 'ruvnet', description: 'Agent meta-harness + swarms (328)', license: 'MIT' },
  { address: 'parcadei/Continuous-Claude-v3', label: 'Continuous Claude', owner: 'parcadei', description: '160 skills — context management', license: 'MIT' },
  { address: 'Orchestra-Research/AI-Research-SKILLs', label: 'AI Research Skills', owner: 'Orchestra-Research', description: 'AI research & engineering library (98)', license: 'MIT' },
  { address: 'tradermonty/claude-trading-skills', label: 'Trading Skills', owner: 'tradermonty', description: '76 skills for equity investors', license: 'MIT' },
  { address: 'browser-act/skills', label: 'Browser-Act Skills', owner: 'browser-act', description: 'Browser automation for agents (71)', license: 'MIT' },
  { address: 'Jeffallan/claude-skills', label: 'Full-Stack Skills', owner: 'Jeffallan', description: '66 full-stack developer skills', license: 'MIT' },
  { address: 'coreyhaines31/marketingskills', label: 'Marketing Skills (Corey Haines)', owner: 'coreyhaines31', description: '44 marketing skills', license: 'MIT' },
  { address: 'EveryInc/compound-engineering-plugin', label: 'Compound Engineering', owner: 'EveryInc', description: 'Compound engineering skills (Every)', license: 'MIT' },
  { address: 'Yeachan-Heo/oh-my-claudecode', label: 'Oh My ClaudeCode', owner: 'Yeachan-Heo', description: 'Multi-agent orchestration (40)', license: 'MIT' },
  { address: 'kepano/obsidian-skills', label: 'Obsidian Skills', owner: 'kepano', description: 'Agent skills for Obsidian (kepano)', license: 'MIT' },
  { address: 'pbakaus/impeccable', label: 'Impeccable', owner: 'pbakaus', description: 'Design language for agent-built UIs', license: 'Apache-2.0' },
  { address: 'Leonxlnx/taste-skill', label: 'Taste', owner: 'Leonxlnx', description: 'Gives your AI good design taste', license: 'MIT' },
  { address: 'nextlevelbuilder/ui-ux-pro-max-skill', label: 'UI/UX Pro Max', owner: 'nextlevelbuilder', description: 'Design-intelligence skill', license: 'MIT' },
  { address: 'OthmanAdi/planning-with-files', label: 'Planning With Files', owner: 'OthmanAdi', description: 'Persistent file-based planning', license: 'MIT' },
];
...
export async function listFeaturedMarketplaces(): Promise<Array<FeaturedMarketplace & { added: boolean }>> {
...
function filterCatalogItems(items: CatalogItem[], opts: ItemQuery): CatalogItem[] {
  ...
  return items.filter((it) => {
    ...
    if (!q) return true;
    return `${it.name} ${it.title} ${it.description ?? ''} ${it.categories.join(' ')}`.toLowerCase().includes(q);
  });
}
...
async function readExternalReadme(entry: CatalogEntry): Promise<string | null> {
  ...
  try {
    const loaded = await loadItem(
      { registry: ext, item: entry.item.name, raw: `${entry.registry}:${entry.item.name}` },
      githubLoaderOptions,
    );
...
export async function getCatalogItemDetail(id: string): Promise<CatalogItemDetail | null> {
  ...
  const files = (entry.item.files ?? []).map((f) => ({ target: f.target ?? f.path, type: f.type }));
  ...
  const dependencyItems: DependencyItem[] = (base.dependencies ?? []).map((name) => {
    ...
    return found
      ? { id: found.id, name: found.name, type: found.type, title: found.title, description: found.description }
      : { id: name, name, type: 'registry:skill', title: name, description: null };
  });
