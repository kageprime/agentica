 ...
 * confirm the now-known user is a member of the Slack workspace's project
 ...
 * asked to link; the bearer proves which user is accepting.
 */
...
slackIdentityApp.openapi(
  createRoute({
    ...
    path: '/bind',
    tags: ['channels'],
    summary: 'Bind the calling user to a Slack user from a /login token',
    ...auth,
  ...
  async (c: any) => {
    ...
    // The workspace must be connected to at least one project, and the
    ...
    if (projectIds.length === 0) {
      return c.json({ error: 'This Slack workspace is not connected to any project.' }, 403);
    }
    ...
    // Link regardless of membership. Connecting your account is decoupled
