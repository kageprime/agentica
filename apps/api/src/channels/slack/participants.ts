  ...
  return [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: `*${escapeMrkdwn(input.requesterLabel)}* wants to join this session.\nThis Slack thread is private until you approve them.`,
      },
    },
    {
      type: 'actions',
...
async function postJoinRequest(input: {
  ...
  await postEphemeral(
    ...
    input.channel,
    ownerSlackUserId,
    `${label} wants to join this session.`,
...
export async function decideSlackThreadJoin(input: {
  ...
  if (!decider) return { ok: false, text: 'Connect your account before approving session access.' };
  ...
  const [session] = await db
    ...
    .limit(1);
  if (!session) return { ok: false, text: 'This session no longer exists.' };
  ...
  if (token) {
    const text = input.decision === 'approved'
      ? `You've been approved for this session. Send your message again and I'll continue.`
      : 'The session owner declined your request for this session.';
    await postEphemeral(
      token,
      input.channelId,
      input.requesterSlackUserId,
  ...
  return {
    ok: true,
    text: input.decision === 'approved'
      ? `Approved ${label} for this session.`
      : `Denied ${label} for this session.`,
  };
}
