...
// Resolve the Slack sender to the user they linked via `/login`, and
...
// A re-link (same Slack user, new user) overwrites the mapping and clears
...
//   • unlinked   → "Connect your account" (opens the /login web flow)
  ...
  return [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: 'The platform needs a linked account before it can run from Slack. Connect or create one to continue. _Only you can see this._',
      },
    },
    {
      type: 'actions',
  ...
  return [
    {
      ...
      text: {
        type: 'mrkdwn',
        text: "You're connected, but your account doesn't have access to this project yet. Request access and an admin will approve it. _Only you can see this._",
      },
    },
    {
      type: 'actions',
...
export async function postIdentityPrompt(input: {
  ...
  if (input.reason === 'unlinked') {
    ...
    fallback = 'The platform needs a linked account to continue.';
  } else {
...
export async function notifyAdminsOfAccessRequest(input: {
  ...
  const blocks = [
    {
      type: 'section',
      text: { type: 'mrkdwn', text: `${text}\nOpen *Members* in the platform to approve.` },
    },
    {
      type: 'actions',
      elements: [
        {
          type: 'button',
          text: { type: 'plain_text', text: 'Review in the platform', emoji: true },
          style: 'primary',
          url: projectUrl,
...
// Reverse of lookupSlackIdentity: the Slack user a user is linked to in a
