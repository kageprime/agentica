...
async function handleReview(
  ...
  if (!identity) {
    return cardResponse(buildNoticeCard('Connect your account (`/login`) to act on reviews.'));
  }
...
async function handleRequestAccess(
  ...
  switch (outcome.status) {
    ...
    case 'pending':
      ...
      return cardResponse(buildNoticeCard('Access requested. An admin will approve it in the platform.', '✅'));
    case 'already-member':
      return cardResponse(buildNoticeCard("You already have access — send your message again and I'll pick it up."));
    case 'no-identity':
      return cardResponse(buildNoticeCard('Connect your account first, then request access.'));
    case 'no-project':
