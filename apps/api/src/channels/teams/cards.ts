...
export function buildFinalCard(opts: {
  ...
  if (opts.sessionUrl) {
    elements.push({
      type: 'TextBlock',
      text: `[Open session in the platform ↗](${opts.sessionUrl})`,
      wrap: true,
      isSubtle: true,
      size: 'small',
      spacing: 'medium',
...
export function buildAnswerCard(body: string, sessionUrl?: string): Record<string, unknown> {
  ...
  if (sessionUrl) {
    elements.push({
      type: 'TextBlock',
      text: `[Open session in the platform ↗](${sessionUrl})`,
      wrap: true,
      isSubtle: true,
      size: 'small',
      spacing: 'medium',
...
export function buildPanelCard(opts: {
  ...
  const actions = opts.url ? [openUrlAction('Open in the platform', opts.url)] : undefined;
...
export function buildReviewCard(opts: {
  ...
  if (opts.viewUrl) actions.push(openUrlAction('View in the platform', opts.viewUrl));
  return card(body, actions);
}
...
export function buildWelcomeCard(opts: { projectUrl?: string }): Record<string, unknown> {
  const body = headerBlock(
    '👋',
    'The platform is connected here',
    '@-mention me with a task and an agent gets on it — replying right here with live progress. Type `/help` to see what I can do.',
  );
  const actions = opts.projectUrl ? [openUrlAction('Open in the platform', opts.projectUrl)] : undefined;
  return card(body, actions);
}
