/**
 * Shared OpenAPI wiring for the API.
 *
...
export function mountOpenApiDocs(app: OpenAPIHono<any, any, any>, version: string): void {
  app.openAPIRegistry.registerComponent('securitySchemes', 'bearerAuth', {
    ...
    scheme: 'bearer',
    description:
      'Supabase user JWT, or a token: PAT (`kortix_pat_…`), API key (`kortix_…`), service account (`kortix_sa_…`), or (LLM Gateway inference routes only) a project gateway key (`kortix_gw_…`).',
  });
  ...
  app.get('/v1/openapi.json', (c) => {
    const document = app.getOpenAPI31Document({
      ...
      info: {
        ...
        version,
        description:
          'The platform REST API — typed schemas via @hono/zod-openapi. ' +
  ...
  app.get('/v1/docs', Scalar({ url: '/v1/openapi.json', pageTitle: 'API' }));
}
