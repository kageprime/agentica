...
const INIT_INTRO: Line[] = [
  ...
  [
    ...
    t('Agent Terminal', 'fg'),
    t('   '),
    t('·  configure your project', 'faded'),
  ],
  [],
];
...
const AGENT_PICK_INTRO: Line[] = [
  [],
  [t('  Pick your local coding agent to configure this project.', 'dim')],
  [],
  [t('  It picks up the skill — ask it to scaffold triggers,', 'dim')],
...
const initTail = (name: string): Line[] => [
  [],
  [t('Initialized project '), t(`"${name}"`, 'fg'), t(' in '), t(`~/${name}`, 'faded')],
  [t('Wrote 9 files:')],
...
const SCRIPT: Step[] = [
  ...
  { input: "# build your agents locally — they're just files", note: true, out: [] },
  {
    ...
    out: [
      ...
      [],
      [t('  '), t('kortix ship', 'kortix'), t('  new project → managed git', 'dim')],
      [t('  name    ', 'dim'), t('my-app')],
      [],
...
const PALETTE: { cmd: string; desc: string }[] = [
  { cmd: 'kortix init', desc: 'scaffold a new project' },
