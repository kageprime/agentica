 ...
 * Output text mirrors the real `agentica` CLI so the terminal reads as authentic.
 * ─────────────────────────────────────────────────────────────────────────── */
  ...
  projects: [
    {
      ...
      files: 9,
      branch: 'main',
      repo: `git.dosco.live/acme/${PROJECT}`,
      url: `dosco.live/p/${PROJECT}`,
    },
  ],
...
export const SCRIPT: Command[] = [
  /* 1 ── agentica init → a draft project appears on the Projects tab ─────────── */
  {
    input: `agentica init ${PROJECT}`,
    beats: [
      nav('projects'),
      wait(260),
      blank(),
      ...
      out([t('Wrote 9 files:')]),
      out([t('  + ', 'faded'), t('agentica.toml')]),
      out([t('  + ', 'faded'), t('.agentica/opencode/agents/agentica.md')]),
      out([t('  + ', 'faded'), t('.claude/skills/agentica/SKILL.md')]),
  ...
  /* 2 ── agentica ship → the draft flips shipping → live ─────────────────────── */
  {
    input: 'agentica ship',
    beats: [
      nav('projects'),
      ...
      okLine(t('agentica.toml verified')),
      blank(),
      out([t('  '), t('agentica ship', 'agentica'), t('  new project → managed git', 'dim')]),
      out(meta('name', PROJECT, 'fg')),
      blank(),
      wait(450),
      okLine(t('Committed: '), t('agentica: ship', 'fg')),
      ...
      out(meta('repo', `git.dosco.live/acme/${PROJECT}`)),
      out([t('  live  ', 'dim'), t(`dosco.live/p/${PROJECT}`, 'cyan')]),
      wait(1050),
    ],
  },
  ...
  {
    input: 'agentica connectors connect linear',
    beats: [
      nav('integrations'),
      wait(260),
  ...
  {
    input: 'agentica connectors connect notion',
    beats: [
      nav('integrations'),
      wait(220),
      out([t('  Open this URL to authorize:', 'dim')]),
  ...
  {
    input: 'agentica providers login openai',
    beats: [
      nav('models'),
      wait(260),
  ...
  {
    input: `agentica sessions new --prompt "Summarize this week's pipeline…"`,
    beats: [
  ...
  {
    input: `agentica chat --prompt "What changed in our repos since Monday?"`,
    beats: [
      okLine(t('session '), t('repo-changes', 'fg'), t(' · streaming', 'dim')),
      nav('chat'),
      fx((a) => a.runChat(chat('repos'))),
      wait(2000),
      out([t('  ▸ ', 'cyan'), t('github.list_commits ', 'fg'), t('dosco-inc/suna', 'faded')]),
      wait(2500),
  ...
  {
    input: `agentica chat --prompt "Run the weekly finance report and email the team…"`,
    beats: [
  ...
  {
    input: `agentica triggers add daily-briefing --cron "0 8 * * *"`,
    beats: [
      nav('scheduling'),
      wait(280),
      fx((a) => a.addSchedule()),
      okLine(t('Added [[triggers]] '), t('daily-briefing', 'fg'), t(' (cron) to agentica.toml')),
  ...
  {
    input: 'agentica secrets set SENDGRID_API_KEY=SG.•••',
    beats: [
      nav('security'),
      wait(280),
      fx((a) => a.addSecret()),
  ...
  {
    input: 'agentica access invite alex@acme.ai --role member',
    beats: [
      nav('security'),
      wait(220),
      fx((a) => a.inviteMember()),
  ...
  {
    input: 'agentica channels connect',
    beats: [
      nav('channels'),
      wait(280),
      ...
      out([t('  webhook  ', 'dim'), t('api.dosco.live/v1/webhooks/slack/1f3a', 'faded')]),
      wait(2400),
    ],
  },
];
