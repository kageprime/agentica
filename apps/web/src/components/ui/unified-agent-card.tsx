...
const CardAvatar: React.FC<{
  ...
  if (isDefaultAgent) {
    return <AgentAvatar isDefault={true} size={size} className="border" />;
  }
...
// Tag list component
const TagList: React.FC<{ tags?: string[]; maxTags?: number }> = ({
  tags,
  maxTags = 3,
}) => (
...
const IntegrationLogos: React.FC<{
  ...
  const toolRequirements = tools.filter(
    (req) => req.source === 'tool' || !req.source,
  );
  const integrations = toolRequirements.filter(
    (tool) => !tool.custom_type || tool.custom_type !== 'sse',
  );
...
const CapabilitiesList: React.FC<{
  capabilities?: string[];
  maxCapabilities?: number;
}> = ({ capabilities, maxCapabilities = 3 }) => (
...
export const UnifiedAgentCard: React.FC<UnifiedAgentCardProps> = ({
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        case 'marketplace':
          return (
            <MarketplaceBadge
              isKortixTeam={data.is_kortix_team}
              isOwner={isOwner}
            />
          );
        case 'template':
    ...
    const renderMetadata = () => {
      ...
      if (
        (variant === 'template' || variant === 'agent') &&
        data.is_public &&
        data.download_count &&
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                ...
                This will permanently remove it from the marketplace and cannot
                be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
