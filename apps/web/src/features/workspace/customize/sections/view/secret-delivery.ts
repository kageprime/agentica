 ...
 *
 *  - `enforced`    — no. The sandbox holds a handle; The platform substitutes the
 ...
 * The platform substitutes it outside on an approved host. `disabled` closes the list
...
const EXPOSURE_PRESENTATIONS: Record<SecretExposure, SecretDeliveryPresentation> = {
  ...
  enforced: {
    ...
    description:
      'The sandbox holds a handle. The platform substitutes the real value only on requests to the approved hosts.',
    tone: 'secondary',
  },
  disabled: {
    label: 'Disabled',
    ...
    description: 'Stored securely, but delivered to no session and no service.',
    tone: 'info',
  },
};
...
> = {
  llm_gateway: {
    label: 'LLM gateway',
    description: 'Spent by the model gateway. It never enters the sandbox.',
    tone: 'secondary',
  },
  connector: {
    label: 'Connector',
