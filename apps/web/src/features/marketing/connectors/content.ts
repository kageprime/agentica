...
export const hero = {
  ...
  sub: 'Connect a tool once, for the whole company. Agents reach it through one scoped token that the platform brokers server-side — so the raw credential never lands in the machine the model is driving.',
...
export const connect = {
  ...
  routes: [
    {
      ...
      body: 'Pick the app, click through its OAuth screen, done. The platform stores the connection, not your password — Gmail, Notion, Linear, Salesforce, HubSpot, Zendesk, Google Drive and thousands more.',
    },
    {
      id: 'custom',
      label: 'Custom',
      title: 'Your own APIs, in the same shape',
      body: 'Point the platform at an OpenAPI or Postman spec, a GraphQL endpoint, a remote MCP server, or a bare HTTP base URL. It reads the source, works out the authentication, and turns every operation into a tool.',
    },
    {
      id: 'channels',
      label: 'Channels',
  ...
  shot: {
    ...
    alt: 'The connector catalogue, showing Notion, Google Sheets, Linear, Google Drive, Salesforce, HubSpot, GitHub, Gmail and more, each one click from connected.',
...
export const broker = {
  ...
  sub: 'A sandbox is a real Linux machine the model can run anything on. So we do not put your credentials in it. The sandbox carries exactly one token, scoped to the project, and every outbound call is assembled on our side of the wall.',
  ...
  after: {
    label: 'How the platform does it',
...
export const policy = {
  ...
  shot: {
    ...
    alt: 'The Permissions tab of the Google Drive connector in the platform: a default rule, then every Drive tool set to Allow, Ask, Block or Default.',
  ...
  pause: {
    ...
    body: 'A gate that errors out teaches an agent to retry around it. A gate holds the call open, so the agent is still mid-task when you answer — and picks up exactly where it stopped.',
    steps: [
...
export const close = {
  ...
  points: [
    'Open source and self-hostable — Cloud, your VPC, or on-prem.',
