...
export const ROLES_TRANSLATION_KEYS: Readonly<Record<string, string>> = {
  ...
  'A gate that errors out teaches an agent to route around it. A gate holds the call open, so you answer while the session is still mid-task and it resumes from exactly where it stopped.':
    'text66d1a4edbd90',
  ...
  'Connect Slack and the shortlist lands in a thread you can argue with. Reply in the same thread and you are talking to the same session, not starting a new one.':
    'textfa4e10e03866',
  'Connected directly from its API description rather than through a middleman: The platform reads the specification, works out the authentication, and turns every operation into a tool with its own answer.':
    'text96fa8ecff679',
  ...
  'Every session gets its own cloud computer and its own branch. The agent reproduces the bug, writes the patch, runs the tests, and opens a change request. Merge is default-deny for agents.':
    'textcec34edc4be8',
  'Every session is a real Linux machine, so an analysis agent can install a package, run the query, and commit the notebook. The analysis lands in the repo as a change request, so it can be re-run.':
    'text3f5377410e64',
  ...
  'Illustration. The branch name is the session id, because that is what a session branch is.':
    'text69c063f8947f',
  'Illustration. The excluded domain is a placeholder — the platform never names a real customer or tenant.':
    'text3cc0540830e1',
  'Illustration. The paths are the real shape of a project, the copy is fictional.':
    'text97f1a47d1742',
  ...
  'The platform has a real account, member, group and role model with per-resource permissions for people and for agents. An access review that covers your humans and not your automation is half a review.':
    'text895560302007',
  'The platform is open source and self-hostable: Cloud, your own VPC, or your own on-prem network. If personal data may not leave your infrastructure, run the whole platform inside it. For deployment and compliance questions, talk to us rather than trusting a claim on a marketing page.':
    'textba3f27dd11a9',
  ...
  'One rename, one API move, one lint rule turned on — the kind of change that is trivial per file and unbearable at scale. It works file by file on its own branch and lands one reviewable diff.':
    'text2c0f44757d70',
  'Open source and self-hostable. Any model, your keys. Cloud, your own VPC, or your own on-prem network.':
    'text8af666ebb4c8',
  ...
  'Plan against actual per cost centre, then the part people actually want: which transactions drove the variance, and a written explanation of each one that a non-finance reader can follow.':
    'text0450eaa713ef',
  'Point the platform at an OpenAPI or Postman spec, a GraphQL endpoint, a remote MCP server, or a bare HTTP base URL. It reads the source, works out the authentication, and turns every operation into a tool the agent can call.':
    'text41fb6b8b43f6',
  ...
  'Reached as a connector you define: an OpenAPI or Postman description, a GraphQL endpoint, a raw HTTP base URL, or a remote MCP server. The platform reads the source, works out the authentication, and turns each operation into a tool with its own Allow, Ask or Block.':
    'text4610c1dbee14',
  ...
  'Set the send action to Ask and the run holds at the call, showing you the message and who it is addressed to. Approve and the same call completes. Deny and the session carries on without it.':
    'text10e0b5222505',
  'Single sign-on is SAML 2.0. Every tool call is written to an audit record with the agent, the person or trigger, the outcome and the approver. Deployment is Cloud, your own VPC, or your own on-prem network — it is open source, so you can read what you are running. It is not air-gapped: starting a self-hosted stack pulls images over the network. For an isolated topology, talk to us.':
    'text07d514c3e52e',
  ...
  'The sandbox carries one project-scoped token and no third-party keys. The banking or ledger credential is decrypted server-side and attached to the outbound request, then thrown away.':
    'text40a3454eacbe',
  'The sandbox carries one project-scoped token and no third-party keys. The credential for your campaign tool is decrypted server-side and attached to the outbound request.':
    'text21d978d10cd9',
  'The sandbox carries one project-scoped token and no third-party keys. The gateway decrypts the real credential server-side and attaches it to the outbound call. A runtime secret you deliberately grant is different — that one is a real environment value the agent can read, and it is meant to be.':
    'text58f7e09c0d5f',
  'The sandbox carries one project-scoped token and no third-party keys. Your applicant-system credential is decrypted server-side and attached to the outbound request, then thrown away.':
    'text1b8f0674b970',
  'The sandbox carries one project-scoped token and no third-party keys. Your CRM credential is decrypted server-side and attached to the outbound request. Turning a connector off takes effect on the next call — there is nothing in the sandbox to rotate.':
    'textd13a607ddf4b',
  'The sandbox carries one project-scoped token and no third-party keys. Your tracker and helpdesk credentials are decrypted server-side and attached to the outbound call.':
    'textd864380a766f',
  ...
  'The workbook is derived, not typed. Each line carries the query or the document it came from, so the review question is "is this the right source" rather than "where did this number come from".':
    'text8fb82f06ce29',
  'This is most of an IT estate. Point the platform at an OpenAPI or Postman description, a GraphQL endpoint, a bare HTTP base URL, or a remote MCP server. It reads the source, works out the authentication, and turns each operation into a tool with its own Allow, Ask or Block.':
    'textd6a7bfaf08b0',
