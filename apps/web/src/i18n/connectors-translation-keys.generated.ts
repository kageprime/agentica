...
export const CONNECTORS_TRANSLATION_KEYS: Readonly<Record<string, string>> = {
  ...
  'A gate that errors out teaches an agent to retry around it. A gate holds the call open, so the agent is still mid-task when you answer — and picks up exactly where it stopped.':
    'textc87684446059',
  'A hash of the arguments, and a redacted result — never a raw secret.': 'text5ef80ac814da',
  'A sandbox is a real Linux machine the model can run anything on. So we do not put your credentials in it. The sandbox carries exactly one token, scoped to the project, and every outbound call is assembled on our side of the wall.':
    'text17ca81a2d026',
  ...
  'Connect a tool once, for the whole company. Agents reach it through one scoped token that the platform brokers server-side — so the raw credential never lands in the machine the model is driving.':
    'text58aede554b21',
  ...
  'Open source and self-hostable — Cloud, your VPC, or on-prem.': 'texta6e6f5c3b55b',
  ...
  'Pick the app, click through its OAuth screen, done. The platform stores the connection, not your password — Gmail, Notion, Linear, Salesforce, HubSpot, Zendesk, Google Drive and thousands more.':
    'textf57829f4ec88',
  'Point the platform at an OpenAPI or Postman spec, a GraphQL endpoint, a remote MCP server, or a bare HTTP base URL. It reads the source, works out the authentication, and turns every operation into a tool.':
    'textde8b3da50590',
  ...
  'The gateway that resolves the credential is also the thing that writes the record. There is no path to a connected tool that skips it.':
    'text40c42e206ab5',
  'The connector catalogue, showing Notion, Google Sheets, Linear, Google Drive, Salesforce, HubSpot, GitHub, Gmail and more, each one click from connected.':
    'text0845712d2765',
  ...
  'The Permissions tab of the Google Drive connector in the platform: a default rule, then every Drive tool set to Allow, Ask, Block or Default.':
    'text169a667df8ce',
