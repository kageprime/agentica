...
Agents can authenticate to the protected API with a user-authorized OAuth access token or a Dosco personal access token.
...
Audience: an AI agent acting for an existing user.
...
1. Install the CLI from ${KORTIX_WEB_ORIGIN}/install.
2. Run \`kortix login\`.
3. Open the registration URI generated by the CLI.
4. Sign in as the user who owns the agent action.
...
The platform does not accept anonymous agent registration, ID-JAG assertions, or automated verified-email claims.
`;

export const KORTIX_PUBLIC_CONTENT_SKILL = `---
name: kortix-public-content
description: Discover and read public documentation through standards-based HTTP and MCP endpoints.
---

# Public Content

Use this skill to discover APIs and public product documentation.
    ...
    skills: [
      {
        name: 'kortix-public-content',
        type: 'skill-md',
        description: 'Discover and read public API and documentation resources.',
...
export const MCP_SERVER_CARD = {
  ...
  description: 'Search and read public documentation and API metadata.',
