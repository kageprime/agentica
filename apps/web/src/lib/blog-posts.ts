...
const agiReadyArchitecture: BlogPostEntry = {
  ...
  title: 'AGI-ready architecture: what it really means, and how the platform is built for it',
  description:
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'This is the single most important invariant, and it is the one most “agent platforms” violate. If the model’s context window is where your state lives, you have no state — you have a lucky streak that ends when the session ends. In the platform, everything is files in a git repo. The manifest, the agents, the skills, the connectors, the policies, and the memory are all versioned, diffable, owned files. The model proposes; a trusted subsystem persists.',
    },
    {
      type: 'code',
...
triggers:
  - slug: nightly-access-review
    ...
    {
      type: 'p',
      text: 'Authentication answers who the agent is. Authorization answers what it may do — and the model must never be the final authority on the second question. In the platform, every session runs under a scoped identity with a single token carrying claims for principal, project, session, and agent grant. Connector credentials are bound server-side and injected at runtime; they never enter the sandbox environment, the transcripts, or the model’s view.',
    },
    {
      type: 'ul',
      items: [
    ...
    {
      type: 'p',
      text: 'An intelligent agent should never execute directly inside the control plane. In the platform, every session is its own isolated Linux sandbox on its own git branch — a disposable machine the agent owns, with filesystem, network, and process isolation. Thousands run in parallel on the same config without colliding, because none of them share state. Egress and credentials are controlled at the network boundary, and the sandbox assumes the agent-generated code is untrusted even when the model looks reliable.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'The platform is the Autonomous Company Operating System — open-source, self-hostable, any model. Start one project free.',
    },
  ],
};
...
git project -> session -> isolated computer + session branch
    ...
    {
      type: 'p',
      text: 'QM starts with the social graph. A person, Slack channel, group message, or project room gets a durable scope. That scope owns memory, files, credentials, schedules, skills, and a computer. The platform starts with the work graph. A project is a git repository and `kortix.yaml`; each session receives a sandbox and a branch, and durable changes return through a change request.',
    },
    {
      type: 'callout',
      text: 'QM is closest to a persistent AI colleague for every person and room. The platform is closest to a versioned operating system where many agents work on isolated branches of the company.',
    },
    { type: 'h2', text: 'What they genuinely share' },
    {
      ...
      items: [
        ...
        '**Background work** — QM has crons and watches; The platform has cron and signed-webhook triggers.',
    ...
    {
      type: 'p',
      text: 'The platform treats `@kortix/sdk` as a public product boundary. `createKortix({ getToken })` returns one client for project and session lifecycle, files, streaming, runtime health, previews, and errors. React hooks, a TypeScript server entry, the real `kortix` CLI, mobile, desktop, and the web app build on that package. The API and OpenCode transport are implementation details for host applications.',
    },
    {
      type: 'p',
...
npm exec qm -- outputs --json`,
    ...
    { type: 'h2', text: 'Scaling, observability, and performance' },
    {
      ...
      items: [
        ...
        '**No honest benchmark winner yet** — the projects publish different tests and target different lifecycles. QM should be faster on repeated tool use in one warm scope. The platform should isolate parallel work more cleanly and amortize boot through provider snapshots and resume. Those are architectural expectations, not an apples-to-apples measured result.',
      ],
    },
    { type: 'h2', text: 'Licensing is not a footnote' },
    {
      type: 'p',
      text: 'QM is MIT-licensed. You can modify it, redistribute it, and build a hosted service from it under the MIT terms. The platform uses the **Elastic License 2.0**. You can inspect, modify, and self-host the source, but the license restricts providing the software to third parties as a competing hosted or managed service. Some enterprise functionality also requires a license entitlement.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'They could coexist, but no supported connection exists today. QM could own the conversational scope while the platform runs project sessions and change requests behind it. That adapter would have to preserve identity, grants, delivery provenance, and idempotency across both systems. Treat it as a connector project, not a configuration flag.',
    },
    ...
    },
    {
      ...
      body: 'Create a project, start an isolated session, and review the change your agent brings back. Self-host it or use Cloud.',
    },
  ],
};
...
const introducingKortix: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'The platform is the **command center for the AI agents that do your work** — one place to build a workforce of agents, connect them to your tools, run them on your terms, and keep every result accountable to a human.',
    },
    {
      type: 'p',
    ...
    {
      type: 'p',
      text: 'In the platform, a **project** is one git repository. The repo *is* the project: its files, its history, its agents, its automations, its settings — all of it lives in git. Start fresh with a private repo hosts for you, or bring an existing one on GitHub.',
    },
    {
      type: 'ul',
      items: [
    ...
    {
      type: 'p',
      text: 'At the root of every project sits one file: `kortix.yaml`. Any repo with a valid manifest at its root *is* a project — that file defines what the project is, what it’s allowed to do, and how it runs. Here’s a real one:',
    },
    {
      type: 'code',
...
connectors:
  - slug: slack
    ...
    {
      type: 'p',
      text: 'When AI becomes how your company gets work done, the system running it stops being a tool and becomes infrastructure. Infrastructure you don’t own can be changed, repriced, or switched off without your say. So the platform is **open-source and self-hostable**, and you can run the entire stack on your own infrastructure — one command brings up a production-style Dosco on your own machines, and the same CLI switches between our cloud and yours.',
    },
    {
      type: 'p',
...
const kortixVsClaudeCowork: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'They can even coexist: a power user keeps Cowork on their desktop while the company runs its shared, governed workforce on the platform.',
    },
    {
      type: 'cta',
      title: 'Love agents that do the work? Run a whole fleet — on your own terms.',
      body: 'Connect your tools and hand an agent a real task. Free to start, free to self-host.',
    },
  ],
};
...
const beyondTheChatBox: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'The point isn’t that chat assistants are bad. They’re excellent at what they’re built for, and they belong in the stack. An agent that manages a vendor risk review might start by asking a chat assistant to digest a SOC 2 report — then take that output and run the full workflow. The key is knowing which tool fits which job:',
    },
    {
      type: 'ul',
      items: [
        ...
        '**Code completion in-IDE.** Tools like Claude Code and Cursor are brilliant at diffing, refactoring, and writing code in your editor. Agents orchestrate those same tools at scale.',
    ...
    },
    {
      ...
      body: 'Hand an agent a real task and get a finished result back. Free to start, free to self-host.',
    },
  ],
};
...
const secureAiAgentToolAccess: BlogPostEntry = {
  ...
  blocks: [
    ...
    { type: 'h2', text: 'How models a connector' },
    {
      type: 'p',
...
agents:
  ...
  release-bot:
    ...
    {
      type: 'p',
      text: 'That is the model behind the Connector. Every session gets a scoped Connector token. The agent discovers tools, describes their schemas, and calls them through the API. The gateway enforces the project grant and connector policy, resolves credentials outside the sandbox, runs the request, and audits the call. The [connections guide](/docs/guides/connecting-tools) is explicit: the agent never holds third-party credentials.',
    },
    {
      type: 'callout',
...
└─ docs/                    # source-of-truth operating docs`,
    ...
    {
      type: 'p',
      text: 'Those are not abstract use cases for us. The platform runs internal sweeps for production errors, PR review, docs maintenance, weekly briefs, outbound research, and this SEO/blog loop from the same project-native model: agents with skills, memory, tools, triggers, and a reviewed path for durable changes.',
    },
...
const kortixVsGlean: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'They can coexist, too. Plenty of companies will keep Glean as the search layer and run the work itself on the platform — agents that read, decide, and act, with the operating layer they need to do it governed as code. (The desktop side has its own parallel: [how the platform compares to Claude Cowork](/blog/kortix-vs-claude-cowork).) If that operating layer is what you’re missing, the [company OS post](/blog/ai-transformation-company-os) and the [secure connector model](/blog/secure-ai-agent-tool-access) are the next reads.',
    },
    {
      type: 'cta',
...
const kortixVsPoetic: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'What the platform does own is narrower and, we would argue, the part that actually needs owning: the boundary where that code touches the outside world. Every connector call goes through one server-side chokepoint, the Connector gateway, and the typed client is part of [`@kortix/sdk`](https://www.npmjs.com/package/@kortix/sdk).',
    },
  ...
  if (!result.ok) throw new Error(\`close_dispute \${dispute.id}: \${result.reason}\`);
    ...
    },
    {
      ...
      items: [
        ...
        '**You can self-host it.** The platform is open source. Run it on your own infrastructure with your own keys and your own models. This is not an air-gapped story — `kortix self-host start` pulls images and reaches a sandbox provider over the network — but the data, the config and the model are yours.',
    ...
    { type: 'h2', text: 'Where this comparison does not favour us' },
    {
      type: 'ul',
      items: [
        '**No determinism claim.** Poetic compiles to deterministic execution. A session is a model loop. On a single fixed process at extreme volume, that is their win, not ours.',
...
const openSourceAiManagementSystem: BlogPostEntry = {
  ...
  blocks: [
    {
      type: 'lead',
      text: 'The platform is the open-source AI Management System — the leading open-source alternative to Claude Cowork and ChatGPT Work. One git repo holds the agents, the skills, the memory and the connector config. Every session runs on its own isolated machine, on its own branch. Any model, your keys. Work lands through a change request. Self-host it, or run it on our cloud. This is the long version of that paragraph: every layer, in the order you meet it, with the parts we do not claim marked as such.',
    },
    ...
    {
      type: 'code',
      code: `# kortix.yaml — the layer of the repo.
kortix_version: 2
runtime: opencode
...
→ Active host is now cloud`,
    ...
    },
    {
      ...
      items: [
        ...
        '**One harness, one runtime.** The platform runs OpenCode. That is the shipped path, and it is the only one this post describes.',
      ],
    },
    { type: 'h2', text: 'When to pick which' },
    {
...
const theOnlyMoatThatMatters: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'This is the ethos open-source has always represented: platforms that enable more value on top than they capture inside. The platform is open-source, self-hostable, and model-agnostic by design. We want every company that uses Dosco to build its own compounding advantage — not to make Dosco the only company that gets smarter.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: "The platform is the open-source AI operating system where your company's knowledge compounds. Connect your tools, deploy an agent, and start accumulating your own token capital. Free to start, free to self-host.",
    },
  ],
};
...
const twoKindsOfCapital: BlogPostEntry = {
  ...
  blocks: [
    ...
    },
    {
      ...
      body: 'The platform is the open-source AI OS where your company\u2019s knowledge compounds. Free to start, free to self-host, free to own your learning loop.',
    },
  ],
};
...
const staticSoftwareIsDead: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'The sandbox is the feedback loop. Every session is an isolated environment with a deterministic signal. The agent tries, observes, and iterates. Skills are the accumulated output of that loop \u2014 compressed experience, not static code. This is why we invest in the sandbox architecture. It is the foundation.',
    },
    {
      type: 'cta',
      title: 'Stop building static software. Start building feedback loops.',
      body: 'The platform is the platform for dynamic software. Deploy on-prem, own your data, and build systems that learn.',
    },
  ],
};
...
const theConvergenceOfAiProducts: BlogPostEntry = {
  ...
  blocks: [
    ...
    },
    {
      ...
      body: 'The platform is built on the feedback loop. Deploy it, use it, and watch it compound. Start building yours.',
    },
  ],
};
...
const frontierEcosystem: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'The platform is open-source, self-hostable, and model-agnostic for exactly this reason. We want every company to build its own compounding advantage on top of AI, not rent it from someone else. The data flywheel that makes your company better over time should belong to you, not to a model provider in San Francisco.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'The platform is open-source and built for companies that want to own their AI future. Self-host it, connect your own models, and build something that compounds.',
    },
  ],
};
...
const goodBusinessesDontNeedMoats: BlogPostEntry = {
  ...
  blocks: [
    ...
    {
      type: 'p',
      text: 'The platform is open-source. Anyone can clone the repo. Anyone can self-host. Anyone can build a competing product. There is no moat in the VC sense.',
    },
    {
      type: 'p',
      text: 'But the cloud business is real. The brand is real. The open-source community is real. The customer relationships are real. And most importantly, the **data flywheel** is real \u2014 every company that uses the platform builds a compounding advantage that belongs to them, not to us. That is not a moat around Dosco. It is a moat around our customers.',
    },
    {
      type: 'p',
    ...
    },
    {
      ...
      body: 'The platform is the platform for companies that want to own their AI learning loop. Self-host it, connect your models, and build something that compounds over time.',
    },
  ],
};
