...
We run an HR-policy helpdesk agent on the platform that reads the inbox every 15
...
On the platform, a cron fires every 15 minutes and spawns a fresh session with
