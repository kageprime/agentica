...
We run a payment-recovery agent on the platform that watches every failed Stripe
...
On the platform, an hourly cron re-prompts one **persistent session** that keeps a
