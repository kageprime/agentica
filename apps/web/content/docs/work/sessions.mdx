...
A session is one unit of agent work. The platform cuts a git branch and provisions
...
| Status | Meaning |
|---|---|
| `provisioning` | The platform cuts the branch and requests the sandbox. |
...
The platform does not write them for a session today.
...
Deleting a session destroys its sandbox for good. The platform keeps the session
...
Every session uses OpenCode REST. The platform stores the selected OpenCode agent and
...
The platform runs sessions on Daytona, Platinum, or E2B Cloud. A project follows
