...
The platform does work inside a [session](/docs/work/sessions): a branch and a sandbox for one
...
1. The platform creates the session row and cuts a branch from the default branch.
   The branch name is the session id.
2. The platform resolves a sandbox image: the default image, or your own
...
4. The agent works. It uses [secrets](/docs/project/secrets) through the
   environment variables the platform sets, then commits and pushes to the session
   branch.
