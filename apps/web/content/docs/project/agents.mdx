...
- **Governance** — an entry in `kortix.yaml`'s `agents` map, keyed by the same
  name. It sets what the agent may access on the platform: connectors, secrets,
...
OpenCode loads a skill on demand when the agent calls it — the platform does not
...
Creating a session accepts an `agent_name`. The platform starts that agent for the
session. If you omit `agent_name`, the platform uses the project's `default_agent`.
