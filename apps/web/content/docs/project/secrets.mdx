---
title: Secrets
description: How the platform stores project credentials, and how it controls whether agent code can read each value.
---
...
The platform stores secrets on the project, never on the account, and encrypts
...
| **None** | Nothing at all | A credential only a service spends, or a value kept on file and disabled |
...
- **Sent secrets** — the value travels on the wire. API keys, bearer tokens,
  ...
  bytes in an outbound request, so the platform can put it there itself, outside the
...
`Authorization: Basic <base64>`). The platform cannot find a handle that is embedded,
...
| A URL scheme other than HTTPS | The platform terminates TLS to substitute |

`api.example.com` does not cover `uploads.api.example.com`. Add the second
host to the same secret. The platform rejects an unenforceable policy with `400`
...
A host on the list presents a certificate the platform issued for this sandbox, which
...
An approved host is reached through the platform, so it behaves a little differently
from a direct call:
...
- Only clients that honour `https_proxy` are intercepted. The platform sets that
...
For a request the platform cannot intercept, the agent has an explicit door to the
...
code can read it, print it, and forward it anywhere. The platform cannot redact it,
...
- **Personal override** — your own value for one name, used instead of the
  shared row for sessions you start. Today the platform uses this only for one OAuth
  login credential.

The platform never lets an LLM provider key, for example `ANTHROPIC_API_KEY`,
...
any secret a service spends, needs its identifier written out in an
...
egress-enforced secret, so the platform writes an explicit list: every identifier the
...
:::warning[Secret policies are not a multi-tenant authorization system]
...
that surface shares one project agent, one grant, and one host list. The platform has
...
which records. Secret exposure is one layer below it, and it does not
substitute for it.
...
<Step title="The platform pushes it to running sessions">
The platform pushes the change to every sandbox with an active session for the
...
sandbox holds a handle, and the platform reads the current value when the next
...
only adds permissions — the platform has no deny rule — so no role can withhold either
...
exact name `CODEX_AUTH_JSON` with `400` — the platform manages that secret
...
A secret write does not wait for a session restart. The platform pushes the
change to every active sandbox in the project:

1. The platform builds a new environment snapshot, using the running agent's
   `secrets` grant.
2. The sandbox writes the snapshot to the live agent environment. New tool
   calls pick up the change right away.
3. If the changed secret is an LLM provider credential, the platform restarts
   OpenCode.
...
A project on the platform's managed model access needs no key of its own. To bring
