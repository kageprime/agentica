...
An account holds your [projects](/docs/project) and the people who work in them. When you sign up, the platform creates a personal account for you. Invite a teammate and it becomes a team account. Personal and team accounts use the same roles, billing, and limits.

## One access model

The platform has one grant record: an **assignment**. Every assignment binds one principal to one role, at one scope.

| Term | Meaning |
|---|---|
| **Principal** | Who holds the access. A `user`, a `group`, a `service_account` (an agent's identity), or a `pending` invitee email. |
| **Role** | A named set of permissions. The platform ships the built-in roles below. An account can add custom roles. |
...
An assignment can name one object inside a project: an `agent`, a `skill`, a `secret`, an `app`, or a `trigger`. The platform enforces object assignments on agents and skills today.
...
A person, a group, or an agent gets access from the platform as **roles** — the assignments described above. An agent carries a **second, separate binding**: the CLI scopes its manifest declares in `kortix.yaml` under `agents.<name>.kortix_cli`. A session can only do what both allow. The two never widen each other. An agent whose manifest lists `project.secret.read` still reads nothing when the role verdict denies it, and an agent launched by an owner still reads nothing when its manifest does not list the action. Roles are account state; CLI scopes are repository state in the manifest. See [Manifest reference](/docs/project/manifest#agents).
...
A custom role only adds permissions. The platform has no deny rule.
...
Branding follows the account, not the browser: inside a project, members see the brand of the account that owns the project. Sign-in pages, emails, and public share pages stay Dosco, because there is no account to brand until someone is signed in. When the Enterprise entitlement lapses, members see Dosco again — nothing is deleted, and the account can remove what it uploaded at any time.
...
| `DELETE /v1/accounts/{accountId}/branding` | Reset everything to the platform. `account.write` only. |
...
The platform signs you in with a personal access token (`kortix_pat_...`). It acts as the user who created it and holds exactly that user's assignments. A service account (`kortix_sa_...`) is a separate principal, not a person's credential, and holds no project access until an assignment gives it some.
...
Billing applies at the account level and covers every project in the account. The platform offers a free tier, a pro tier, and a per-seat team tier, each with its own model access and credits. Using your own model key does not make usage free: A platform fee still applies on top of it, except on the free tier. For what a credit buys, the per-service markups, and how to make a balance last, see [Credits & usage](/docs/credits).
