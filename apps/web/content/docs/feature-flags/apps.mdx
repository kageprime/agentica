---
title: Apps
description: Deploy static sites, bundles, Dockerfiles, and OCI images to stable URLs.
---

An App is a provider-neutral serverless deployment owned by one project.
...
All four source kinds run on the same sandbox hosting backend. Kortix
selects Daytona, Platinum, or E2B. They therefore share one deployment contract
and one cold-wake contract.

| Kind | Deploy this | The platform does |
|---|---|---|
...
Cloud it is `<env>-<slug>-<route-key>.apps.kortix.com`. Self-hosted
...
Access uses a five-minute exchange URL and an eight-hour, host-only,
...
current App runtime. The old deployment keeps serving while Kortix
