...
The platform supports four channel platforms today:
...
The platform links each chat sender to an account before the agent runs for
...
page. The platform never injects it into a sandbox. The platform resolves the token
...
On a host with the shared Slack app configured (Cloud, for example),
...
commands do not run inside the Assistant DM pane, so the platform parses the same
