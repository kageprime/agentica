...
calls it as a tool. The platform brokers each call, so the sandbox never holds the
connector credential.

You declare most connectors in `kortix.yaml`. See the
[manifest reference](/docs/project/manifest) for every field. The platform declares
...
Permission: an agent reaches a connector only when its manifest grant
...
The platform encrypts connection data and resolves it server-side for each tool
call. The agent requests an action. The platform attaches the credential, checks the
...
not grant authority. The page requires a signed-in account. The platform then
...
Approve or deny the call once. The platform sends the decision back into the session
...
Complete the OAuth flow. The platform stores the connected account as a connection.
...
2. The platform reads the protected resource metadata (RFC 9728) at that URL, or at
   `/.well-known/oauth-protected-resource`.
3. The platform reads the authorization server metadata (RFC 8414 or OpenID Connect
...
create no application, and you copy no client ID or secret. The platform then runs
...
The platform keeps the MCP session: it runs `initialize` and
...
the callback carries an `iss` parameter (RFC 9207), the platform rejects it unless it
...
The platform encrypts the OAuth2 configuration and tokens. It refreshes access tokens
...
The platform reads the manifest from the default branch. The connector becomes
active after the change request merges.

</Step>
<Step title="Add the connection">
Open the connector and set its credential. The platform stores the value
