...
- **The name** is declared in your manifest (`agentica.toml`). Safe to commit — it's only a label like `OPENAI_API_KEY`.
...
In `agentica.toml`, add the name under `[env]`. List it as `required` if the agent can't work without it, or `optional` if it's nice to have:
...
Secrets are encrypted at rest with AES-256-GCM and injected as plain environment variables into the sandbox at session boot. Names must be env-var-shaped, and the `AGENTICA_*` prefix is reserved for platform values. From the CLI: `agentica secrets set NAME=VALUE` upserts a value, `agentica secrets ls` lists what's declared and set, and `agentica env push --from .env` bulk-uploads a dotenv file. See the [secrets reference](/docs/reference/secrets).
</Callout>
