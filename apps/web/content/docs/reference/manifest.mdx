...
`agentica.toml` lives at the repo root. Any repo with a valid `agentica.toml` at the root is a Agentica project. The parser is permissive — it never throws on a bad entry: bad triggers and apps go into an `errors` list alongside the good ones, and unknown top-level tables are ignored (park your own metadata freely).
...
name = "ML Development"
dockerfile = ".agentica/Dockerfile.ml"   # repo-relative; or `image = "python:3.12-slim"`
cpu = 4
memory = 16
...
`agentica_version` is the required schema version. A manifest declaring a version higher than the platform knows about is rejected outright — the platform won't silently misread future fields. When the platform writes the manifest back after a dashboard edit, it ensures `agentica_version` is the first key.
...
| Apps deploy sweep | `[[apps]]` (when `AGENTICA_APPS_EXPERIMENTAL=true`)        |
...
Declares the env vars your sessions need. Values live in the Agentica Secrets Manager, never inline; the platform decrypts and injects them as plain env vars at session start.
...
dockerfile = ".agentica/Dockerfile.ml"   # repo-relative
cpu        = 4                          # vCPU cores
memory     = 16                         # GiB
disk       = 50                         # GiB
...
| Field        | Type   | Default            | Notes                                                  |
| ------------ | ------ | ------------------ | ------------------------------------------------------ |
| `config_dir` | string | `.agentica/opencode` | Repo-relative dir. Same silent-fallback behavior as `[[sandbox.templates]]` paths. |

That directory holds agents, skills, slash commands, tools, plugins, and `opencode.jsonc` — the layout opencode reads by convention. OpenCode owns the runtime semantics; Agentica may inspect metadata to build server-side agent/model UI surfaces. See [Agentica vs OpenCode config](/docs/reference/config-boundary) and [opencode.ai/docs](https://opencode.ai/docs/).
...
Do not duplicate those details in `agentica.toml`. Use `[[agents]]` for the
Agentica-side decision of which agents are launchable and what server-side grants
they receive.
...
- **computer** — connected machines over the Agent Computer Tunnel.
  **Synth-only: you cannot declare it in `agentica.toml`.** Connecting a machine
...
store, not in `agentica.toml`. Auth is not allowed with `provider = "pipedream"`.
...
- Keep `agentica_version` as the first key.
