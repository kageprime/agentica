---
title: Sandbox image
description: The layered Docker image, what Agentica injects, constraints, examples, and snapshot rebuilds.
---

The sandbox base image. For how a session uses it at runtime, see [Session runtime](/docs/reference/session-runtime).

Every session boots inside a Docker container, built in two parts: **your Dockerfile** defines the environment (languages, tools, system packages), and the **Agentica runtime layer** is auto-injected on top so the dashboard can connect.

```
┌─────────────────────────────────────────┐
│  Agentica runtime layer (auto-injected)   │  ← opencode + agentica-agent + ENTRYPOINT
├─────────────────────────────────────────┤
│  Your Dockerfile                        │  ← .agentica/Dockerfile
└─────────────────────────────────────────┘
```

Referenced from `agentica.toml` as a named template under `[[sandbox.templates]]`:

```toml
[[sandbox.templates]]
slug       = "dev"             # any slug except the reserved "default"
name       = "Dev box"         # optional label
dockerfile = ".agentica/Dockerfile"   # OR image = "python:3.12-slim"
...
4. A baked OpenCode tool-dependency cache under `/opt/agentica/` (so the boot-time `bun install` in your config dir is a no-op, not a network round-trip).
5. `COPY` the `agentica-agent` daemon, the `agentica` CLI, and the `agentica-entrypoint` script to `/usr/local/bin/`, plus the `slack-cli` and `executor-sdk` package trees under `/opt/agentica/`.
6. `ENV AGENTICA_WORKSPACE=/workspace`, `WORKDIR /workspace`, `EXPOSE 8000`.
7. `ENTRYPOINT ["/usr/local/bin/agentica-entrypoint"]`.
...
| Rule | Why |
| --- | --- |
| Don't set `ENTRYPOINT`. | Agentica overrides it. Your `CMD` is also ignored. |
...
| Don't `RUN apt-get clean` without `rm -rf /var/lib/apt/lists/*`. | The Agentica layer re-runs `apt-get update`; a broken lists cache breaks it. |
