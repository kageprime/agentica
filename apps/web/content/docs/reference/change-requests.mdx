...
Agentica's pull-request equivalent — the path session work takes to reach `main`. For the plain-language version see [Change requests](/docs/concepts/change-requests); for a review walkthrough see [Reviewing and merging](/docs/quickstart).

A CR proposes merging one branch (`head_ref`) into another (`base_ref`) inside a single Agentica project. The layer is Agentica-native — it works on any git host (GitHub, GitLab, plain git) without per-host integration. The CR row is metadata; the git operations (fetch, diff, three-way merge, fast-forward) run inside the Agentica API against the project's `repo_url` backend.
...
1. **Commit on the session branch** (`$AGENTICA_BRANCH_NAME`). Small, working commits. Don't rewrite or force-push.
2. **Push the branch** (`git push origin HEAD`).
3. **Open the CR** (`agentica cr open --title "…" --description "…"`). From inside the sandbox `--head` and `--session` are auto-detected from `$AGENTICA_BRANCH_NAME` and `$AGENTICA_SESSION_ID`; `--base` defaults to the project's default branch.
4. **Surface the CR number to the user** (`agentica cr ls`) so they can review.
5. **Stop. The agent does not merge its own CR.** Merging is the user's call, from the dashboard or `agentica cr merge <n>`.

Applies to everything: code edits, new files (skills, agents, slash commands, tools, plugins), `agentica.toml` edits (triggers, env, sandbox, apps), `AGENTS.md` changes, new MCP server configs — anything committed to the tree. There is no "small enough to skip the CR" exception.
...
The unique index on `(project_id, number)` lets the CLI accept `agentica cr show 3` — `3` resolves to the row with `number = 3` for the project. Numbers don't recycle on close; the counter keeps going.
...
```
                       open
                        │
                        ├── Agentica cr close ──▶  closed ──┐
                        │                          │      │
                        │                  Agentica cr reopen
                        │                          │      │
                        │   ◀──────────────────────┘      │
                        │                                 │
                        ├── Agentica cr merge  ──▶  merged ─┘ (terminal)
```

- `open` is the starting state.
- `closed` is reversible: `agentica cr reopen <n>` puts it back to `open`. The branch tips are re-resolved on the next read.
...
`agentica cr diff` falls back to no color when stdout isn't a TTY (or with `--no-color`).
...
2. Otherwise create a merge commit (three-way merge). Default message `Merge CR #<n>: <title>`; override with `--message`. Author is `agentica <noreply@agentica.dev>`.
...
`agentica cr show <cr>` calls this automatically for open CRs and renders it inline. Run `show` before `merge` to see what you're about to do.
...
3. Commit + push.
4. `agentica cr show <cr>` to confirm the preview is now clean.
...
| --- | --- |
| `agentica cr ls [--status open\|merged\|closed\|all]` | List CRs on the project. Default: `open`. |
| `agentica cr show <cr>` | Metadata + merge preview. |
| `agentica cr diff <cr> [--no-color]` | Unified patch. |
| `agentica cr open --title "..." [--description "..."] [--head <ref>] [--base <ref>]` | Open a CR. |
| `agentica cr merge <cr> [--message "..."]` | Merge it (fast-forward or three-way). |
| `agentica cr close <cr>` | Close without merging. |
| `agentica cr reopen <cr>` | Reopen a closed CR. |

`<cr>` is the per-project number (`3` or `#3`) or the UUID `cr_id`.

### Sandbox auto-detection

When `agentica cr open` runs inside a session sandbox:

- `--head` defaults to `$AGENTICA_BRANCH_NAME` (or `$AGENTICA_HEAD_REF`).
- `--session` defaults to `$AGENTICA_SESSION_ID`, which back-fills `origin_session_id` on the row so the dashboard can show which session opened the CR.
- `--base` defaults to the project's default branch (from `projects.default_branch`, usually `main`).
- `--project` defaults to the session's project (from `$AGENTICA_PROJECT_ID`).
- `--title` is the only required flag.

Minimal viable invocation in a sandbox:

```sh
agentica cr open --title "Add release-notes skill" \
...
| --- | --- |
| **Sessions** | A CR's `origin_session_id` is back-filled from `$AGENTICA_SESSION_ID` so the dashboard shows the session that opened it. Cascade is `ON DELETE SET NULL` — closing the session orphans the link, doesn't delete the CR. |
| **Skills** | New `.agentica/opencode/skills/<name>/SKILL.md` files reach future sessions **only** after a CR that contains them merges to `main`. Until then, only the originating session sees them. |
| **Agents** | Same: new `.agentica/opencode/agents/<agent>.md` files need to land via CR. |
| **Triggers** | Edits to `[[triggers]]` in `agentica.toml` only reach the scheduler after the CR merges. The scheduler reads `agentica.toml` on `main`. |
...
| **Secrets** | Decoupled. Secrets live in the Agentica Secrets Manager, not the manifest; CRs don't move secrets. |
...
- **Merge is `agentica <noreply@agentica.dev>`-authored.** If you want the user's name on the merge commit, that's a dashboard-side option, not a CLI flag today.
...
- **The `AGENTICA_*` env vars expected at `cr open` time:** `AGENTICA_TOKEN`, `AGENTICA_API_URL`, `AGENTICA_PROJECT_ID`, `AGENTICA_BRANCH_NAME` (or `AGENTICA_HEAD_REF`), `AGENTICA_SESSION_ID`. All are pre-injected by the session bootstrap. Running `agentica cr open` outside a session needs `--head` and `--project` explicitly, or a cwd linked via `agentica projects link`.
