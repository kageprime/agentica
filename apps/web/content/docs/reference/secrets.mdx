...
1. **Declare a secret name** in `agentica.toml`:
...
2. **Set the value** in the Agentica Secrets Manager (dashboard) — or via `agentica secrets set` / `agentica env push`.
...
- The `AGENTICA_*` prefix is reserved for platform variables; user secrets cannot use it. The Secrets Manager CRUD endpoint rejects it. The manifest parser does not enforce this, so declaring `AGENTICA_FOO` in `[env]` is accepted but no matching secret can be created. Don't.
...
The dashboard's Secrets Manager (and `agentica secrets ls`) shows which secrets are declared in `agentica.toml` (`required` or `optional`) and which have a value set, marking required-but-missing names.
