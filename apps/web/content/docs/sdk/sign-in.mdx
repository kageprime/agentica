---
title: Sign in with Dosco
description: Gate your own app behind Dosco identity with one route, and act as the signed-in user through the SDK.
---

"Sign in with Dosco" makes Dosco the identity provider for an app you run:
a dashboard, an internal tool, a vertical product built on the platform. Your users
sign in with their account, your server knows who they are, and every
API call your app makes runs as that user with that user's role
assignments. The whole flow lives in `@kortix/sdk`. Your app never stores a
Token in the browser and never talks to Supabase.

It is standard OAuth 2.1 (authorization code + PKCE) served by the API,
so it works the same against `api.dosco.live` and against a self-hosted
instance.

:::note
  Building an App **hosted by Dosco** (`*.apps.kortix.com`)? You need none of
...
| `kortix` | Acting as the user on the whole API — projects, sessions, files, IAM probes. Without it the token is identity-only. |
...
| Path | Does |
|---|---|
| `/signin?return_to=/path` | Starts sign-in (PKCE S256 + state in a 10-minute cookie) and redirects to the platform. |
...
| `/proxy/*` | Forwards to the API as the viewer. The browser SDK's `backendUrl`. |
...
The first time, Dosco shows a consent screen naming your app and the scopes.
Dosco remembers the decision per user and app, so later sign-ins redirect
straight back. Revoking an app deletes every token it minted.

## Discovery

`GET https://api.dosco.live/.well-known/oauth-authorization-server` (also
