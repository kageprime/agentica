---
title: Apps
description: Create, deploy, and control Apps from the SDK and from React.
---
...
An App hosted by the platform is opened by someone Kortix **already signed in**. The
...
which the platform injects into your App at deploy. A forged header never passes: the
...
One setting on the App's access policy — **Settings → Access** in the platform, or
...
| `api` | The above, and a token that acts **as** that person on the API — bounded by their own role. |
| `off` | Nothing. |

The token is never the user's session: it lasts an hour, carries only
those scopes, and every token an App minted dies when the App is deleted or its
access policy changes. `public` and `password` Apps have no signed-in the platform
viewer, so they receive none of this.

An App served on its **own domain** (not `*.apps.kortix.com`) has no gate in
front of it — use [Sign in with Dosco](/docs/sdk/sign-in) there instead.
