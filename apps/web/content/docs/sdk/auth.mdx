...
## OAuth access tokens (Sign in with Dosco)
...
whole lifecycle — see [Sign in with Dosco](/docs/sdk/sign-in).
...
Every ordinary sign-in flow is available through the API, so a CLI, a
native app, a script, or your own backend signs users up and in without a
Supabase URL or key — on dosco.live and on a self-host alike.
...
| `auth.user(token)` / `auth.signOut(token)` | `GET /v1/auth/user`, `POST /v1/auth/sign-out` | Sign-out revokes at Supabase and in the session gate. |
...
API yet — it stays on the web app.
...
| The CLI | `kortix login` (mints a personal access token) |
...
| Your own app, signed in by its users with their account | [Sign in with Dosco](/docs/sdk/sign-in) — an OAuth access token the SDK manages for you |
