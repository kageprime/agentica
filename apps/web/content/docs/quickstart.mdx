...
The platform supports macOS and Linux. There is no Windows binary yet.
</Step>
...
`kortix ship` creates the project in Cloud on its first run, then
pushes your code. Run `kortix ship` again any time to sync local changes.

To work on a project that already exists in Cloud, clone it instead:
