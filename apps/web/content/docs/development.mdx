...
description: Run the Agentica stack on your machine and work with the encrypted API secrets.
---
...
| `pnpm dev:dev-env` | **dev** | the **dev** stack — dev Supabase DB, **test** Stripe, dev keys (mirrors `dev-api.dosco.live`). |
| `pnpm dev:staging-env` | **staging** | the **staging** stack — staging Supabase DB and staging keys (mirrors `staging-api.dosco.live`). |
| `pnpm dev:prod-env` | **prod** | the **prod** stack — prod Supabase DB, **LIVE** Stripe, prod keys (mirrors `api.dosco.live`). |
